...
```
Other entity properties are described below.

### CustomView
customView are used to specify custom layout for UI. When property are undefined it means to use default autogenerated layout for entity view.
...
customView: "board"
```
Needs you to create `board.jade` file whith jade template source code inside.
